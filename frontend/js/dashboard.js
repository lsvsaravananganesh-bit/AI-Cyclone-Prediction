...
function updateDashboard(data=demo){
  ...
  $('latitude').textContent=`${Number(data.latitude||0).toFixed(2)}°N`;
  $('longitude').textContent=`${Number(data.longitude||0).toFixed(2)}°E`;
}
function buildChart(){
...
async function refreshData(){
  const btn=$('refresh'); btn.disabled=true; btn.classList.add('loading');
  try{const response=await fetch('/api/prediction',{headers:{Accept:'application/json'}});if(!response.ok)throw new Error('API unavailable');updateDashboard(await response.json());}
  catch{updateDashboard(demo);}
  finally{$('lastUpdate').textContent='Updated just now';btn.disabled=false;btn.classList.remove('loading');}
}
$('refresh')?.addEventListener('click',refreshData);
updateDashboard();buildChart();
