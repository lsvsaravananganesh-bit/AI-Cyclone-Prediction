...
CYCLONE_MODEL_PATH / CYCLONE_TRACK_MODEL_PATH for trained models.
"""
...
app=FastAPI(title="AI Cyclone Prediction API", version="1.3.0")
...
def health():
    return {"status":"ok","service":"cyclone-intelligence-api","time":datetime.now(timezone.utc).isoformat(),
            ...
            "image_model":bool(os.getenv("CYCLONE_MODEL_PATH")),"track_model":bool(os.getenv("CYCLONE_TRACK_MODEL_PATH"))}

def _imd_get():
    headers={"Accept":"application/json","User-Agent":"AI-Cyclone-Prediction/1.3"}
...
@app.post("/api/ml/predict-track")
async def predict_track(payload:dict):
    model_path=os.getenv("CYCLONE_TRACK_MODEL_PATH")
    if not model_path or not os.path.exists(model_path):
        return {"model_status":"MODEL_NOT_TRAINED","prediction":None,"message":"Train ml/train_track_model.py and configure CYCLONE_TRACK_MODEL_PATH."}
    try:
        import joblib
        import pandas as pd
        artifact=joblib.load(model_path); model=artifact["model"]; features=artifact["features"]
        row={k:payload.get(k) for k in features}
        if any(v is None for v in row.values()): raise ValueError("Missing required features: "+", ".join(k for k,v in row.items() if v is None))
        x=pd.DataFrame([row]); delta=model.predict(x)[0]
        lat=float(payload["latitude"])+float(delta[0]); lon=float(payload["longitude"])+float(delta[1]); wind=max(0,float(payload["wind"])+float(delta[2]))
        return {"model_status":"MODEL_OUTPUT","model_version":os.path.basename(model_path),"prediction":{"latitude":round(lat,5),"longitude":round(lon,5),"wind":round(wind,2)},"confidence":None,"message":"One-step forecast generated by the trained ML track model. Validate with held-out storms before operational use."}
    except Exception as e: return {"model_status":"MODEL_ERROR","prediction":None,"message":str(e)}
...
async def analyze_image(file:UploadFile=File(...)):
    ...
    if not model_path or not os.path.exists(model_path): return {"model_status":"MODEL_NOT_TRAINED","classification":None,"confidence":None,"demo":False,"message":"Image validated. Install a trained model artifact before requesting a prediction."}
    try:
        import torch
        from torchvision import models, transforms
        checkpoint=torch.load(model_path,map_location="cpu",weights_only=False); classes=checkpoint["classes"]
    ...
    except Exception as e: return {"model_status":"MODEL_ERROR","classification":None,"confidence":None,"demo":False,"message":f"Model could not be loaded: {type(e).__name__}"}
